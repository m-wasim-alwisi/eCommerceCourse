		...
		<div class="container">
			<form class="form-horizontal" action="?do=Insert" method="POST">
				...
				<div class="form-group form-group-lg">
					...
					<div class="col-sm-10 col-md-6">
						<div class="radio">
							<label><input type="radio"   name="MyRadio" value="1" checked>Admin</label>
						</div>
						<div class="radio">
							<label><input type="radio"   name="MyRadio" Value="0">Not Admin</label>
						</div>
					</div>
				</div>
				<!-- End Full Name Field -->
			...
			if (empty($formErrors)) {
				...
				if ($check == 1) {
					...
					$stmt = $con->prepare("INSERT INTO 
												...
												VALUES(:zuser, :zpass, :zmail, :zname,:GroupID, 1, now()) ");
					$stmt->execute(array(
		...
		if ($count > 0) { ?>
			...
			<div class="container">
				<form class="form-horizontal" action="?do=Update" method="POST">
					...
					<div class="col-sm-10 col-md-6">
						<div class="radio">
							<label><input type="radio" name="MyRadio" value="1"  <?php  echo ($row['GroupID']=='1') ? 'checked' : ''; ?>>Admin</label>
						</div>
						<div class="radio">
							<label><input type="radio" name="MyRadio" Value="0"  <?php  echo ($row['GroupID']=='0') ? 'checked' : ''; ?>>Not Admin</label>
						</div>
					</div>
				</div>
					<!-- End Full Name Field -->
		...
		if ($_SERVER['REQUEST_METHOD'] == 'POST') {
			...
			if (empty($formErrors)) {
				...
				if ($count == 1) {
					...
					$stmt = $con->prepare("UPDATE users SET Username = ?, Email = ?, FullName = ?, GroupID=? , Password = ? WHERE UserID = ?");

					$stmt->execute(array($user, $email, $name,$MyRadio, $pass, $id));
